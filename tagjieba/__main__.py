...
parser.add_argument("-V", '--version', action='version',
                    version="Jieba " + tagjieba.__version__)
parser.add_argument("filename", nargs='?', help="input file")

args = parser.parse_args()

if args.quiet:
    tagjieba.setLogLevel(60)
if args.pos:
    import tagjieba.posseg
    posdelim = args.pos
    def cutfunc(sentence, _, HMM=True):
        for w, f in tagjieba.posseg.cut(sentence, HMM):
            yield w + posdelim + f
else:
    cutfunc = tagjieba.cut
...
if args.dict:
    tagjieba.initialize(args.dict)
else:
    tagjieba.initialize()
if args.user_dict:
    tagjieba.load_userdict(args.user_dict)
